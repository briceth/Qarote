...
import { notionConfig, isProduction } from "@/config";
...
class NotionService {
   ...
   */
  private async getDataSourceId(): Promise<{
    success: boolean;
    dataSourceId?: string;
    error?: string;
  }> {
   ...
   */
  async createUser(params: CreateNotionUserParams): Promise<{
    success: boolean;
    notionPageId?: string;
    error?: string;
  }> {
   ...
   * Updated for Notion API 2025-09-03: uses data_source_id instead of database_id
   */
  async findUserByUserId(userId: string): Promise<{
    success: boolean;
    notionPageId?: string;
    error?: string;
  }> {
   ...
   */
  async syncUser(user: User): Promise<{
    success: boolean;
    notionPageId?: string;
    error?: string;
  }> {
