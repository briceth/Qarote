...
export class Paths {
  ...
  static get sshKeyPath(): string {
    const keyPath = `${process.env.HOME || ""}/.ssh/id_rsa_deploy`; // id_rsa_deploy is for local
  ...
  static get sshKeyPublicPath(): string {
    const publicKeyPath = `${process.env.HOME || ""}/.ssh/id_rsa_deploy.pub`; // id_rsa_deploy is for local
